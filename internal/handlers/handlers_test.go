...
var theTests = []struct {
	...
	{"show res", "/admin/reservations/new/?id=1", "GET", http.StatusOK},
...
func TestAvailabilityJSON(t *testing.T) {
	for _, e := range testAvailabilityJSONData {
		...
		var req *http.Request
		if e.postedData != nil {
			req, _ = http.NewRequest("POST", "/search-json", strings.NewReader(e.postedData.Encode()))
		} else {
			req, _ = http.NewRequest("POST", "/search-json", nil)
		}
		ctx := getCtx(req)
		req = req.WithContext(ctx)
		req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func TestPostAvailability(t *testing.T) {
	for _, e := range testPostAvailabilityData {
		req, _ := http.NewRequest("POST", "/search", strings.NewReader(e.postedData.Encode()))
