...
var theTests = []theTest{
	...
	// },
	// 	http.StatusOK},
	// {"post-search-avail-json", "/search-json", "POST", []postData{
	// 	{key: "start", value: "2020-01-01"},
	// 	{key: "end", value: "2020-01-02"},
	// },
	// 	http.StatusOK},
	// {"post-search-avail-json", "/make-reservation", "POST", []postData{
...
func TestRepository_PostReservation(t *testing.T) {
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler should redirect to home page because there is no reservation type in session: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Test for bad form parse reservation in session
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler should redirect to home page because form data is bad: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Test for bad reservation insert
	req, _ = http.NewRequest("POST", "/make-reservation", strings.NewReader(reqBody))
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	reservation.RoomID = 2 // failing tests
	session.Put(ctx, "reservation", reservation)
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler should redirect to home page because reservertion insert failed: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Test for bad room restriction insert
	req, _ = http.NewRequest("POST", "/make-reservation", strings.NewReader(reqBody))
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	reservation.RoomID = 99 // failing tests
	session.Put(ctx, "reservation", reservation)
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler should redirect to home page because room restricition insert failed: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
}
...
func TestRepository_AvailabilityJSON(t *testing.T) {
	...
	if err != nil {
		t.Errorf("could not get json response from POST request")
	}

	if !j.OK {
		t.Errorf("Server returned an failed request for searching availability.")
	}
	...
	if j.OK {
		t.Errorf("Server returned an passed request for searching availability when it should fail.")
	}
}
